...
# ====================== 8. 保存训练好的模型 ======================
# torch.save() 将 ResNet-18 的全部参数（约 1170 万）保存到磁盘
# 注意：ResNet 参数量大，.pt 文件约 45MB（远大于 MLP 的 ~350KB）
torch.save(model.state_dict(), "04_resnet_digital.pt")
print("模型已保存到 04_resnet_digital.pt")

# ====================== 9. 查看训练后各层权重的统计信息 ======================
# ResNet-18 有几十个参数层，遍历 named_parameters() 可以检查每一层：
#   - 权重均值/标准差：判断训练是否收敛
#   - 梯度消失征兆：深层权重接近 0
#   - 梯度爆炸征兆：权重数值极大（ResNet 有 BN 层，一般不会爆炸）
print("\n==================== 各层权重统计 ====================")
for name, param in model.named_parameters():
    if 'weight' in name:
        print(f"{name:40s} | 均值: {param.mean():+.6f} | "
              f"标准差: {param.std():.6f} | "
              f"最小值: {param.min():+.6f} | "
              f"最大值: {param.max():+.6f}")
print("========================================================\n")